...
public class PacbotTeleop implements ITeleop
{
	...
	public boolean turnRadiusRunning = false;
...
//				 ActuatorConfig.getInstance().getLeftTalonFront().set(ControlMode.Velocity, LtargetVelocity_UnitsPer100ms);
				 ...
				 //Bobby suggested that these functions be added to joystick capabilities (4, 5, 2, and 3)
/*				 else if(leftJoystick.getRawButton(4)){//be careful-- you cannot interrupt the process
					 turnRadiusRunning = true;
					 drivetrain.turnRadiusCancel = false;
					 ActuatorConfig.getInstance().getDrivetrain().turnLeftRadius(.30, 90, 0, 24 ); //,45
				 }
				 else if(leftJoystick.getRawButton(5)){//be careful-- you cannot interrupt the process
					 turnRadiusRunning = true;
					 drivetrain.turnRadiusCancel = false;
					 ActuatorConfig.getInstance().getDrivetrain().turnRightRadius(.23, 90, 0, 24 );
				 }	*/
				 else if (leftJoystick.getY() > 0.15 || rightJoystick.getY() > 0.15 || leftJoystick.getY() < -0.15 || rightJoystick.getY() <-0.15)
				 {
					 if(turnRadiusRunning = true)
						 turnRadiusRunning = false;
						 drivetrain.turnRadiusCancel = true;
/* 					//self correction
